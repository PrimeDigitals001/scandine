...
- **Deferred (later):** **Phase 2** shared-table CUSTOMER UI (the RPCs + super-admin seat QR already support it; needs the `/court` flow to pass the seat token + a `?s=` friend-join + "table in use" already handled by `FcMenuLoader`); **Phase 3** cross-store "your orders at this court" view + court tab bar. `setup_hosted.sql` still not re-synced (migrations are canonical).
